...
import com.user.fund.service.TokenVerificationService;

@Path("/funds")
public class fundResource {

	fundRepository repo = new fundRepository();
	TokenVerificationService tokenService = new TokenVerificationService();
	...
	public List<fund> getFunds() {
		...
		@Path("fund/{id}")
		@Produces(MediaType.APPLICATION_JSON)
		public Response getPayment(@PathParam("id") int id,@HeaderParam("Authorization")String token,@HeaderParam("uname")String name) {
			boolean isverified = tokenService.getTokenVerificaton(token, name);
			if(isverified) {
				return Response.ok(repo.getFund(id)).build();
			}else {
