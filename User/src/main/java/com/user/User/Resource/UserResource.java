...
public class UserResource {
	...
	@DELETE
	@Path("delete/{id}")
	public Response DeleteUser(@PathParam("id") int id, @HeaderParam("Authcode") String token,
	...
	@GET
	@Path("ver/{token}/{name}")
	public Response verifyToken(@PathParam("token") String token, @PathParam("name") String name) {
		boolean found = tokenSerives.verifyToken(token, name);
		HashMap<String, Object> jData = new HashMap<String, Object>();
		String json = null;
		try {
			jData.put("isValid", found);
			jData.put("name", name);
			json = new ObjectMapper().writeValueAsString(jData);
	 ...
	 * ArrayList<Customer> getallRegisteredCustomer(){
	 * 
	 * 
	 ...
	 * 
	 * @Produces({MediaType.APPLICATION_JSON,MediaType.APPLICATION_XML}) public
	 * Response getuser3(@HeaderParam("Authcode")String
	 * token,@HeaderParam("name")String name,@PathParam("id")int id,User s1) {
	 * 
	 * 
	 * 
	 * 
	 * boolean isAuth = tokenSerives.verifyToken(token, name); if(isAuth) {
	 * 
	 * cus.CustomerProfile(id); return
	 * Response.status(200).entity(cus.CustomerProfile(id)).build();
	 * 
	 * }else { return Response.status(401).entity("Not Aoth").build(); }
	 * 
	 * }
	 * 
	 * @DELETE
	 * 
	 * @Path("de/{id}") public Response DeleteUser1(@PathParam("id")int
	 * id,@HeaderParam("Authcode")String token,@HeaderParam("uname")String name) {
	 * 
	 * System.out.println("delete called");
	 * System.out.println("sucessfully  deleted"); boolean isAuth =
	 * tokenSerives.verifyToken(token, name); if(isAuth) { Customer a =
	 * cus.CustomerProfile(id); if(a.getId()!= 0) cus.Delete(id); return
	 * Response.status(200).entity("Success").build(); }else { return
	 * Response.status(401).entity("Not Aoth").build(); }
	 * 
	 * }
	 * 
	 * 
	 * @DELETE
	 * 
	 * @Path("de/{id}") public Response DeleteUserPayment(@PathParam("id")int
	 * id,@HeaderParam("Authcode")String token,@HeaderParam("uname")String name) {
	 * 
	 * System.out.println("delete called");
	 * System.out.println("sucessfully  deleted"); boolean isAuth =
